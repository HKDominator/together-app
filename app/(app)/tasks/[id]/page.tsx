...
export default function TaskDetailPage({ params }: Props) {
  ...
  const assignee    = users.find(u => u.id === safeTask.assigneeId)
  const createdBy   = users.find(u => u.id === safeTask.createdById)
  const allowed     = VALID_TRANSITIONS[safeTask.state] ?? []
  const isTerminal  = allowed.length === 0
  const isOverdue   = safeTask.dueDate && new Date(safeTask.dueDate) < new Date() && safeTask.state !== 'done' && safeTask.state !== 'cancelled'

  const STATE_ORDER: TaskState[] = ['todo', 'in_progress', 'done']
  const stateIndex  = STATE_ORDER.indexOf(safeTask.state)

  function transition(newState: TaskState) {
    dispatch({ type: 'SET_STATE', payload: { id: safeTask.id, newState } })
  }

  function handleEdit(formData: ValidatedTaskData) {
    dispatch({ type: 'UPDATE_TASK', payload: { id: safeTask.id, changes: formData } })
  }

  function handleDelete() {
    dispatch({ type: 'DELETE_TASK', payload: { id: safeTask.id } })
    router.push('/tasks')
  }
  ...
  return (
    <div className="p-9">
      ...
      <div className="grid grid-cols-[1fr_320px] gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-9">
          ...
          <div className="flex items-center gap-3 mb-5 flex-wrap">
            <PriorityBadge priority={safeTask.priority} />
            <StateChip state={safeTask.state} />
            {isOverdue && (
          ...
          {/* Title */}
          <h1 className="font-display text-2xl font-bold text-gray-700 mb-4">{safeTask.title}</h1>
          ...
          <div className="flex items-center gap-2 mb-8">
            ...
            {/* Cancelled pill shown separately */}
          ...
          <div className="flex flex-wrap gap-2 mb-8">
            {allowed.includes('in_progress') && (
              <ActionBtn color="blue" onClick={() => transition('in_progress')}>▶ Start Task</ActionBtn>
            )}
            {allowed.includes('done') && (
              <ActionBtn color="green" onClick={() => transition('done')}>✓ Mark Done</ActionBtn>
            )}
            {allowed.includes('cancelled') && (
              <ActionBtn color="gray" onClick={() => transition('cancelled')}>✕ Cancel</ActionBtn>
            )}
            {!isTerminal && (
              <ActionBtn color="crimson" onClick={() => setEditOpen(true)}>✎ Edit Task</ActionBtn>
            )}
          ...
          {!isTerminal && (
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
              ...
              {users.filter(u => u.id !== task.assigneeId).map(u => (
                <button
                  key={u.id}
                  onClick={() => dispatch({ type: 'UPDATE_TASK', payload: { id: task.id, changes: { assigneeId: u.id } } })}
      ...
      {/* Edit modal */}
      <TaskFormModal
        key={safeTask.id} // Reset form when switching between tasks
        isOpen={editOpen}
        task={safeTask}
...
function ActionBtn({ children, onClick, color }: ActionBtnProps) {
  const MAP: Record<ActionBtnColor, string> = {
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold transition-colors ${MAP[color]}`}
    >
      {children}
    </button>
  )
