...
export default function Sidebar() {
  ...
  return (
    ...
    >
      {/* Logo */}
      <div className="px-6 py-7 border-b flex items-center gap-3" style={{ borderColor: 'rgba(232,213,183,0.08)' }}>
      ...
      <nav className="flex-1 py-5">
        <p className="px-6 mb-2 text-xs font-semibold uppercase tracking-widest" style={{ color: 'rgba(232,213,183,0.3)' }}>
          Main
        </p>
      ...
      {/* Workspace users */}
      <div className="px-6 py-5 border-t" style={{ borderColor: 'rgba(232,213,183,0.08)' }}>
        <p className="mb-3 text-xs uppercase tracking-widest font-semibold" style={{ color: 'rgba(232,213,183,0.3)' }}>
          Workspace
        </p>
        {users.map(u => (
          <div key={u.id} className="flex items-center gap-2.5 mb-2.5">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold truncate" style={{ color: 'rgba(232,213,183,0.85)' }}>{u.name}</p>
              <p className="text-xs capitalize" style={{ color: 'rgba(232,213,183,0.35)' }}>{u.role}</p>
            </div>
            {/* Online dot — Ana is always "online" for demo */}
            <span
              className="w-2 h-2 rounded-full"
              style={{ background: u.id === currentUser.id ? '#27AE60' : '#6B7280' }}
            />
          </div>
        ))}
      </div>
